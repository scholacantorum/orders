...
OrderPayment gets the payment method.  Its properties are:
  ...
  - user - hash with user information
...
<template lang="pug">
#recordings-form-payment(v-show="canPR !== null")
  #recordings-form-use-pr-div(v-if="canPR")
    label#recordings-form-use-pr-label(for="recordings-form-use-pr")
      | Use payment info saved {{ deviceOrBrowser }}?
    b-form-checkbox#recordings-form-use-pr(v-model="usePR" switch)
  div(v-show="!usePR")
    b-form-group.mb-1(
      label="Card number" label-sr-only
      :state="cardError ? false : null" :invalid-feedback="cardError"
    )
      #recordings-form-card.form-control(ref="card")
  #recordings-form-footer
    #recordings-form-message(v-if="message" v-text="message")
    #recordings-form-buttons
      b-btn(type="button" variant="secondary" :disabled="submitting" @click="onCancel") Cancel
      #recordings-form-prbutton(v-show="usePR" ref="prbutton")
      b-btn#recordings-form-pay-now(v-if="!usePR && submitting" type="submit" variant="primary" disabled)
        b-spinner.mr-1(small)
        | Paying...
      b-btn#recordings-form-pay-now(v-if="!usePR && !submitting" type="submit" variant="primary")
...
export default {
  ...
  async mounted() {
    ...
    this.payreq = stripe.paymentRequest({
      ...
      total: { label: 'Schola Cantorum Recording Order', amount: this.total },
  ...
  watch: {
    ...
    usePR() {
      ...
      // because their payment request failed.  If so, the card field is showing
      // error messages because it is empty.  We'll clear that by pretending
  ...
  methods: {
    ...
    async submit() {
      ...
      const error2 = await this.send({
        subtype: 'manual', method: paymentMethod.id,
      })
      this.submitting = false
      this.card.update({ disabled: false })
...
<style lang="stylus">
#recordings-form-use-pr-div
  display flex
  justify-content space-between
  align-items center
  margin 0 -8px 6px 0
#recordings-form-use-pr-label
  margin-bottom 0
  font-size 14px
...
#recordings-form-buttons
  ...
  button
    margin-left 8px
#recordings-form-prbutton
  margin-left 8px
  min-width 110px
#recordings-form-pay-now
