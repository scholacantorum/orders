...
func (tx Tx) FetchOrder(id model.OrderID) (o *model.Order) {
	...
	lrows, err = tx.tx.Query(
		`SELECT id, product, quantity, price, scan, min_used, auto_use FROM order_line WHERE orderid=? ORDER BY id`, o.ID)
	panicOnError(err)
	for lrows.Next() {
		var ol model.OrderLine
		panicOnError(lrows.Scan(&ol.ID, &pid, &ol.Quantity, &ol.Price, &ol.Scan, &ol.MinUsed, &ol.AutoUse))
...
func (tx Tx) SaveOrder(o *model.Order) {
	...
	for _, ol := range o.Lines {
		res, err = tx.tx.Exec(
			`INSERT OR REPLACE INTO order_line (id, orderid, product, quantity, price, scan, min_used, auto_use) VALUES (?,?,?,?,?,?,?,?)`,
			ID(ol.ID), o.ID, ol.Product.ID, ol.Quantity, ol.Price, ol.Scan, ol.MinUsed, ol.AutoUse)
		panicOnError(err)
		if ol.ID == 0 {
