...
form#login(@submit.prevent="onSubmit")
  ...
  b-button#login-button(type="submit" variant="primary" :disabled="disabled" v-text="buttonText")
</template>
...
export default {
  ...
  computed: {
    buttonText() { return this.loggingIn ? 'Logging in...' : 'Login' },
    disabled() { return this.loggingIn || !this.password || !this.username || !this.privs.length },
  },
  methods: {
    async onSubmit() {
